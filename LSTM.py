...
metrics = [BinaryAccuracy(threshold=0.5), Precision(), Recall(), AUC(), TrueNegatives(), TruePositives(), FalseNegatives(), FalsePositives()]


def mult_in_lstm(local_steps, global_steps, n_features, lr, n_layers, loc_n1, loc_n2, glob_n1, glob_n2,
                 n1, n2, n3, drop1, drop2, reg):
    inputA = Input(shape=(local_steps, n_features))
    inputB = Input(shape=(global_steps, n_features))
    local = LSTM(loc_n1, kernel_regularizer=l2(reg), return_sequences=True)(inputA)
    local = LSTM(loc_n1, kernel_regularizer=l2(reg))
    local = Dropout(drop1)(local)
    glob = LSTM(glob_n1, kernel_regularizer=l2(reg), return_sequences=True)(inputB)
    glob = LSTM(glob_n2, kernel_regularizer=l2(reg))(glob)
    glob = Dropout(drop1)(glob)
...
def mult_in_lstm_attn(local_steps, global_steps, n_features, lr, loc_n1, loc_n2, glob_n1, glob_n2,
                 n1, n2, n3, drop1, drop2, reg):
    inputA = Input(shape=(local_steps, n_features))
    inputB = Input(shape=(global_steps, n_features))
    local = LSTM(loc_n1, kernel_regularizer=l2(reg), return_sequences=False)(inputA)
    #local = LSTM(n_neurons2)
    #local = Dropout(drop1)(local)
    local = Attention()([local, local])
    glob = LSTM(glob_n1, kernel_regularizer=l2(reg), return_sequences=False)(inputB)
    #glob = LSTM(glob_n2, kernel_regularizer=l2(reg))(glob)
    #glob = Dropout(drop2)(glob)
    glob = Attention()([glob, glob])
    joined = concatenate([local, glob]) # Attention()([local, glob])
    joined = Dense(n1, activation='relu')(joined)
    joined = Dropout(drop1)(joined)
    joined = Dense(n2, activation='relu')(joined)
#    joined = Dense(n3, activation='relu')(joined)
...
def mult_in_lstm_attn2(local_steps, global_steps, n_features, lr, n_layers, loc_n1, loc_n2, glob_n1, glob_n2,
                 n1, n2, n3, drop1, drop2, reg):
    inputA = Input(shape=(local_steps, n_features))
    inputB = Input(shape=(global_steps, n_features))
    local = LSTM(loc_n1, kernel_regularizer=l2(reg), return_sequences=False)(inputA)
...
def build_lstm(n_steps, n_features, lr, n_layers, n_neurons1, n_neurons2, n_neurons3, n_neurons4, drop1, drop2, reg):
    ...
    if n_layers == 1:
        ...
        model.add(LSTM(n_neurons1, input_shape=(n_steps, n_features), return_sequences=True, kernel_regularizer=l2(reg)))
...
def build_cnn_lstm_attn(local_steps, global_steps, n_features, lr, loc_f1, loc_k1, loc_p1, loc_drop1,
    ...
    local = LSTM(n_neurons1, dropout=loc_drop1, kernel_regularizer=l2(reg))(local)
    ...
    glob = LSTM(n_neurons2, dropout=glob_drop1, kernel_regularizer=l2(reg))(glob)
    joined = Attention()([local, glob])
    joined = Dense(n_neurons3, activation='relu', kernel_regularizer=None)(joined)
    joined = Dropout(drop1)(joined)
    joined = Dense(n_neurons4, activation='relu', kernel_regularizer=None)(joined)
